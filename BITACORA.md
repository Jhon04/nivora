...
**Comandos Rust:** `guardar_documento`, `obtener_documento`, `listar_documentos`,
`eliminar_documento`, `recargar_workspace` (relee `notas/` tras un `git pull`),
`guardar_asset` (bytes base64), `importar_asset` (desde ruta),
`leer_imagen` (bytes en base64, sin guardar: para editar la imagen antes de insertarla).
...
- [ ] **Automatizar git desde la app** (crate `git2`): `pull` al arrancar y `commit`+`push`
      enganchados al autoguardado con debounce (4.10), para no depender de hacer `git pull` a
      mano. Al traer cambios, llamar a `recargar_workspace` y refrescar el listado.
- [ ] **Avisar de las copias en conflicto** en la UI: hoy se ignoran al indexar y solo quedan
      en disco; debería salir "esta nota tiene otra versión" con las fechas para elegir.
